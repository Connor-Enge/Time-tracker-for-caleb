...
export function CalendarView() {
  const { user, shifts, scheduled, addScheduled, deleteScheduled } = useApp();
  ...
  const scheduledByDay = useMemo(() => {
    const m: Record<string, typeof scheduled> = {};
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card>
        ...
        {adding && (
          <form onSubmit={submitSchedule} className="mb-3 space-y-2">
            ...
            >
              <div>
                <div className="text-sm font-medium">
                  {s.startTime}–{s.endTime} <span className="text-xs text-slate-500">scheduled</span>
                </div>
              ...
              <button
                ...
                className="rounded-lg p-2 text-slate-500 hover:bg-amber-100"
                aria-label="Delete scheduled"
              >
                <Trash2 size={16} />
              </button>
            </div>
